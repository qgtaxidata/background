...
public class TaxiAnalyse {
    ...
    @RequestMapping("roadAnalysis")
    public ResultBean<?> getRoadAnalysis(@RequestParam("area")int area, @RequestParam("date")String date){
        return new ResultBean<>(analyseService.getRoadAnalysis(area, date));
    }
}
